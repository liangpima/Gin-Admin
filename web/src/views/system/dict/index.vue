<template>
  <div class="app-container">
        ...
        <el-card class="table-card">
          ...
          <el-table :data="typeList" v-loading="typeLoading" border stripe highlight-current-row @current-change="handleTypeChange">
            ...
            <el-table-column label="操作" width="120">
              <template #default="{ row }">
                <el-button type="primary" link size="small" @click="handleEditType(row as DictTypeItem)">编辑</el-button>
                <el-button type="danger" link size="small" @click="handleDeleteType(row as DictTypeItem)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-card>
        ...
        <el-card class="table-card">
          ...
          <el-table :data="dataList" v-loading="dataLoading" border stripe>
            ...
            <el-table-column label="回显样式" width="100">
              <template #default="{ row }">
                <el-tag :type="tagTypeOf(row.listClass)" size="small">{{ row.listClass || '默认' }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="status" label="状态" width="70">
              <template #default="{ row }">
            ...
            <el-table-column label="操作" width="120">
              <template #default="{ row }">
                <el-button type="primary" link size="small" @click="handleEditData(row as DictDataItem)">编辑</el-button>
                <el-button type="danger" link size="small" @click="handleDeleteData(row as DictDataItem)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-card>
    ...
    <FormDialog v-model="typeDialogVisible" :title="typeDialogTitle" :loading="submitting" @submit="handleTypeSubmit">
      <el-form ref="typeFormRef" :model="typeForm" :rules="typeFormRules" label-width="80px">
        ...
        <el-form-item label="字典类型" prop="type">
          <el-input v-model="typeForm.type" :disabled="isEditType" placeholder="小写字母、数字、下划线，如 sys_user_status" />
        ...
        <el-form-item label="备注"><el-input v-model="typeForm.remark" type="textarea" placeholder="选填" /></el-form-item>
      </el-form>
    </FormDialog>

    <FormDialog v-model="dataDialogVisible" :title="dataDialogTitle" :loading="submitting" @submit="handleDataSubmit">
      <el-form ref="dataFormRef" :model="dataForm" :rules="dataFormRules" label-width="80px">
        <el-form-item label="字典标签" prop="label"><el-input v-model="dataForm.label" placeholder="展示给用户的文本，如「正常」" /></el-form-item>
        <el-form-item label="字典键值" prop="value"><el-input v-model="dataForm.value" placeholder="实际存储的值，如 1" /></el-form-item>
        ...
        <el-form-item label="备注"><el-input v-model="dataForm.remark" type="textarea" placeholder="选填" /></el-form-item>
      </el-form>
    </FormDialog>
  </div>
</template>
...
const dataForm = reactive({ label: '', value: '', sort: 0, listClass: '', status: 1 as number, remark: '' })
...
async function handleTypeSubmit() {
  ...
  try {
    if (isEditType.value) {
      await updateDictType(editingTypeId.value, { name: typeForm.name, status: typeForm.status, remark: typeForm.remark })
    } else {
...
async function handleDeleteType(row: DictTypeItem) {
  ...
  const ok = await ElMessageBox.confirm(`确认删除字典类型「${row.name}」？`, '提示', { type: 'warning' }).catch(() => false)
  if (!ok) return
...
async function handleDeleteData(row: DictDataItem) {
  const ok = await ElMessageBox.confirm(`确认删除字典数据「${row.label}」？`, '提示', { type: 'warning' }).catch(() => false)
  if (!ok) return
